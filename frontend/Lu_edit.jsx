...
function Lu_edit() {
	...
	useEffect(()=> {
		axios.get('http://localhost:8081/getu/'+id)
		.then(res => {
            console.log(res.data.Result.length<=0);
            if(res.data.Result.length<=0){ 
                alert("super admin or admin cant be modified ");
	...
	const handleSubmit = (event) => {
		...
		axios.put('http://localhost:8081/update_u/'+id, data)
		.then(res => {
